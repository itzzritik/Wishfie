...
public class Country extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Bitmap flags[]=new Bitmap[countryNames.length];
        for(int i=0;i<flags.length;i++)
        {
            flags[i]=BitmapFactory.decodeResource(getResources(),R.mipmap.ironman);
        }
        RecyclerView country_list=findViewById(R.id.country_list);
        country_list.setLayoutManager(new LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false));
        country_list.setAdapter(new CountryListAdapter(countryNames,countryAreaCodes,flags));
    }
}
